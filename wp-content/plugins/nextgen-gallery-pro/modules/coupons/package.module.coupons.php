...
class A_Coupon_Admin_Form extends Mixin
{
    ...
    {
        return array('coupon_id_code', 'coupon_discount_type', 'coupon_discount_amount', 'coupon_date_start', 'coupon_date_end', 'coupon_usage_limit');
    }
    ...
    function _render_coupon_usage_limit_field($coupon)
    {
        $coupon->name = 'coupon';
        return $this->_render_number_field($coupon, 'usage_limit', __('Usage limit', 'nextgen-gallery-pro'), $coupon->usage_limit, __('How many times this coupon may be used by users before it becomes invalid. An empty or 0 setting allows infinite uses.', 'nextgen-gallery-pro'), FALSE, '', 0);
    }
...
class C_Coupon extends C_DataMapper_Model
{
    ...
    function validate_current_availability()
    {
        try {
            ...
            $count = $mapper->get_use_count($this->ID);
            if ($this->usage_limit > 0 && $count >= $this->usage_limit) {
                $valid = FALSE;
            }
            if (!$valid) {
                return false;
...
class C_Coupon_Mapper extends C_CustomPost_DataMapper_Driver
{
    ...
    function define($context = FALSE, $not_used = FALSE)
    {
        ...
        $this->define_column('usage_limit', 'DECIMAL');
    }
