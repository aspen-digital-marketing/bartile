...
<form> <?php // no id is necessary, manage_pricelist_page.js will look for the form belonging to the above wrapper ?>
    <input type='hidden'
           name="pricelist_item[{id}][category]"
           data-field-name="category"
           value="<?php echo esc_attr(NGG_PRO_ECOMMERCE_CATEGORY_DIGITAL_DOWNLOADS); ?>"/>
    ...
    <div class="new_pricelist_product_row half_row">
        ...
        <div class="new_pricelist_product_half_row">
            <span><?php echo $i18n['new_product_resolution']; ?></span>
            <input class="resolution_field"
                   type="number"
                   name="pricelist_item[{id}][resolution]"
                   value=""
                   min="0"
                   placeholder="<?php echo esc_attr($i18n['new_product_resolution_placeholder']); ?>"/> px
        </div>
    </div>
</form>